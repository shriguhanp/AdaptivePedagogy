...
class ResearchAgent(BaseAgent):
    ...
    # Class-level semaphore for rate limiting LLM calls (inspired by Deep Research architecture)
    ...
    async def _rate_limited_llm_call(self, coro):
        """
        Wrapper to rate-limit LLM calls using semaphore.
        Inspired by Deep Research's parallel execution architecture and IdeaGen fix.
...
"""
    ...
    async def check_sufficiency(
        ...
        # Wrap LLM call with rate limiter to prevent rate limit errors
    ...
    async def process(
        ...
        while iteration < self.max_iterations:
            ...
            # Skip delay for the first iteration
            if iteration > 1:
                await asyncio.sleep(0.5)  # 0.5 second delay between iterations
