...
class KnowledgeBaseManager:
    ...
    def delete_knowledge_base(self, name: str, confirm: bool = False) -> bool:
        ...
        """
        # We don't check self.list_knowledge_bases() here because we want to allow 
        # deleting KBs that are in config but missing physically, or vice versa (idempotent)
        ...
        if kb_dir.exists():
            try:
                ...
                # We continue even if dir deletion fails (might be partially deleted)

        # 2. Remove from centralized config (the authoritative source)
        try:
            self.config = self._load_config()
            ...
            # (First check centralized config service, then local config)
            try:
                from src.services.config import get_kb_config_service
                kb_config_service = get_kb_config_service()
                if kb_config_service.get_default_kb() == name:
                    # Clear default in centralized service
                    # The next get_default() call will pick a new one
                    kb_config_service.set_default_kb(None)
            except Exception:
                pass
                
            self._save_config()
        except Exception as e:
            print(f"Error updating config during deletion: {e}")
            return False
