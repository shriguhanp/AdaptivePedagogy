...
async def websocket_research_run(websocket: WebSocket):
    ...
    try:
        ...
        if plan_mode in plan_mode_config:
            ...
            # Debug logging for plan_mode configuration
            logger.info(f"[{task_id}] Applied plan_mode '{plan_mode}' configuration:")
            logger.info(f"  planning.decompose: {config['planning'].get('decompose', {})}")
            logger.info(f"  researching.max_iterations: {config['researching'].get('max_iterations', 'NOT SET')}")
        else:
            logger.warning(f"[{task_id}] Unknown plan_mode '{plan_mode}', using defaults")
            logger.info(f"  planning.decompose: {config['planning'].get('decompose', {})}")
            logger.info(f"  researching.max_iterations: {config['researching'].get('max_iterations', 'NOT SET')}")
