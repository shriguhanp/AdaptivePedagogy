...
@router.get("/{kb_name:path}/config")
...
@router.put("/{kb_name:path}/config")
...
async def delete_knowledge_base(kb_name: str):
    ...
    try:
        ...
        logger.error(f"Error deleting KB '{kb_name}': {e}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/{kb_name:path}")
...
@router.post("/{kb_name:path}/upload")
...
@router.get("/{kb_name:path}/progress")
...
@router.post("/{kb_name:path}/progress/clear")
...
async def websocket_progress(websocket: WebSocket):
    ...
    kb_name = websocket.query_params.get("kb_name")
    if not kb_name:
        await websocket.close(code=1003, reason="kb_name query parameter is required")
        return
...
@router.post("/{kb_name:path}/link-folder", response_model=LinkedFolderInfo)
...
@router.get("/{kb_name:path}/linked-folders", response_model=list[LinkedFolderInfo])
...
@router.delete("/{kb_name:path}/linked-folders/{folder_id}")
...
@router.post("/{kb_name:path}/sync-folder/{folder_id}")
