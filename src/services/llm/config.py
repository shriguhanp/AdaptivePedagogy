...
load_dotenv(PROJECT_ROOT / "AdaptiveX.env", override=False)
...
class LLMConfig:
    ...
    binding: str = "openai"
    provider_name: Optional[str] = None  # For provider identification
    api_version: Optional[str] = None
    max_tokens: int = 4096
    temperature: float = 0.7
    top_p: Optional[float] = 0.95  # For Gemini and other providers
    top_k: Optional[int] = 40  # For Gemini and other providers
...
def _get_llm_config_from_env() -> LLMConfig:
    ...
    return LLMConfig(
        ...
        provider_name=binding,  # Use binding as provider name
    )
