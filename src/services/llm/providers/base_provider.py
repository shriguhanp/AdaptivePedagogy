...
from src.utils.network.circuit_breaker import is_call_allowed, record_call_success
...
class BaseLLMProvider(ABC):
    ...
    def __init__(self, config):
        ...
        if self.traffic_controller is None:
            try:
                ...
                # Traffic control is optional - will skip rate limiting if not available
                logger.debug(f"Traffic control not available for {self.provider_name}")
    ...
    async def execute_with_retry(self, func: Callable, *args, max_retries=3, **kwargs):
        ...
        for attempt in range(max_retries + 1):
            try:
                # Use traffic controller if available, otherwise call directly
